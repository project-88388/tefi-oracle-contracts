...
import { bindMethod, typedIdentifier, promiseTypeAnnotation, classDeclaration, classProperty, arrowFunctionExpression, getMessageProperties, convertToQueryMethod } from "./utils";
...
export const createWasmQueryMethod = (jsonschema, responses) => {
  const underscoreName = Object.keys(jsonschema.properties)[0];
  const methodName = camel(underscoreName);
  let responseType = pascal(`${methodName}Response`); // Support naming query responses exactly like the method, or without a preceding "Get".
...
export const createQueryClass = (className, implementsClassName, queryMsg, responses) => {
  ...
  classProperty("client", t.tsTypeAnnotation(t.tsTypeReference(t.identifier("LCDClient")))), // contractAddress
  classProperty("contractAddress", t.tsTypeAnnotation(t.tsStringKeyword())), // constructor
  t.classMethod("constructor", t.identifier("constructor"), [typedIdentifier("client", t.tsTypeAnnotation(t.tsTypeReference(t.identifier("LCDClient")))), typedIdentifier("contractAddress", t.tsTypeAnnotation(t.tsStringKeyword()))], t.blockStatement([// client/contract set
...
export const createExecuteClass = (className, implementsClassName, extendsClassName, execMsg) => {
  ...
  return t.exportNamedDeclaration(classDeclaration(className, [// client
  classProperty("client", t.tsTypeAnnotation(t.tsTypeReference(t.identifier("LCDClient")))), // wallet
  classProperty("wallet", t.tsTypeAnnotation(t.tsUnionType([t.tsTypeReference(t.identifier("Wallet")), t.tsTypeReference(t.identifier("ConnectedWallet"))]))), // contractAddress
  classProperty("contractAddress", t.tsTypeAnnotation(t.tsStringKeyword())), // constructor
  t.classMethod("constructor", t.identifier("constructor"), [typedIdentifier("client", t.tsTypeAnnotation(t.tsTypeReference(t.identifier("LCDClient")))), typedIdentifier("wallet", t.tsTypeAnnotation(t.tsUnionType([t.tsTypeReference(t.identifier("Wallet")), t.tsTypeReference(t.identifier("ConnectedWallet"))]))), typedIdentifier("contractAddress", t.tsTypeAnnotation(t.tsStringKeyword()))], t.blockStatement(blockStmt)), ...methods], [t.tSExpressionWithTypeArguments(t.identifier(implementsClassName))], extendsClassName ? t.identifier(extendsClassName) : null));
};
export const createExecuteInterface = (className, extendsClassName, execMsg) => {
  ...
  t.tSPropertySignature(t.identifier("contractAddress"), t.tsTypeAnnotation(t.tsStringKeyword())), ...methods])));
};
...
export const createPropertyFunctionWithObjectParamsForExec = (methodName, responseType, jsonschema) => {
  const obj = createTypedObjectParams(jsonschema);
  const fixedParams = [identifier("funds", t.tsTypeAnnotation(t.tsTypeReference(t.identifier("Coins"))), true)];
  const func = {
    type: "TSFunctionType",
    typeAnnotation: promiseTypeAnnotation(responseType),
...
export const createQueryInterface = (className, queryMsg, responses) => {
